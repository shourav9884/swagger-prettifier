...
JSONViewer = (function (container) {
    var JSONViewer = function (container) {
        this._dom = {};
...
function getOrCreateHiddenChild(container) {
    var childClassName = "json-container";
    ...
    if (!childFound) {
        const childNodeElement = document.createElement('span');
        childNodeElement.style.cssText = 'display: none;';
        childNodeElement.innerText = container.textContent;
        container.appendChild(childNodeElement);
        childNode = childNodeElement;
    }
    return childNode;
}
...
function addCustomButtons(container) {
    ...
    if (container) {
        ...
        var childNode = getOrCreateHiddenChild(container);
        // Create a new button element
        const prettifyButton = document.createElement('button');
        prettifyButton.innerText = 'Prettify';
        prettifyButton.classList.add('load-json');


        // Define a click event handler for the button
        prettifyButton.addEventListener('click', function () {
            const json = setJSON(childNode.textContent);
            var viewer = new JSONViewer(container);
            viewer.showJSON(json);
            console.log(json);
            // Handle button click event here
        });

        // Append the button next to the Response body
        container.insertAdjacentElement('beforebegin', prettifyButton);
        ...
        // Append the button next to the Response body
        container.insertAdjacentElement('beforebegin', expandButton);
        ...
        collapseButton.addEventListener('click', function () {
            const json = setJSON(childNode.textContent);
            var viewer = new JSONViewer(container);
            viewer.showJSON(json, null, 1);
        });

        // Append the button next to the Response body
        container.insertAdjacentElement('beforebegin', collapseButton);
    }
}

// Function to watch for changes in the DOM
function watchForChanges() {
    console.log("Dom loaded");
    // Select the target node that may change
...
// Add the custom button when the document is ready
